 ...
 * @author guolin
 */
public class Pull2RefreshListView extends LinearLayout {

    private static String TAG = Pull2RefreshListView.class.getSimpleName();
    ...
    public Pull2RefreshListView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        LayoutInflater.from(context).inflate(R.layout.pull_drag_list_view, this, true);
        ...
        listView = (ListView) findViewById(R.id.refreshable_inner_list_view);
...
//        listView.setBottom(this.getHeight() - footer.getHeight());
        ...
        listView.offsetTopAndBottom(-footer.getHeight());
        ...
        if (enalbleDrag2LoadMore && (isAble2Drag() || footer.getHeight() > 0)) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    Log.w("listview", "height:" + listView.getHeight());
...
//                        Log.w(TAG, "layout height:" + headerLayoutParams.height + " height:" + header.getHeight());
     ...
     */
    public void setPull2RefreshListener(Pull2RefreshListener listener, String tag) {
        setEnablePull2Refresh(listener != null);
        mRefreshAction = listener;
        mId = tag;
    }

    public void setDrag2LoadListener(Drag2LoadListener listener) {
        setEnalbleDrag2LoadMore(listener != null);
...
//            Log.w(TAG,"lvp:" + lvp + " (listView.getAdapter().getCount() -1)" + (listView.getAdapter().getCount() -1));
//            Log.w(TAG, "lastChild.getBottom():" + lastChild.getBottom() + "listView.getHeight(): " + listView.getHeight());
...
//                Log.w(TAG, "able to drag up");
        ...
        protected void onPostExecute(Void v) {
            if (currentRefreshingStatus != STATUS_REFRESHING) {
                ...
                if (mRefreshAction != null) {
                    mRefreshAction.onRefresh(Pull2RefreshListView.this);
                } else {
                    Pull2RefreshListView.this.finishRefreshing();
                }
            }
        }
        ...
        protected void onPostExecute(Void v) {
            if (currentLoadingStatus != STATUS_LOADING) {
                ...
                if (mDrag2LoadAction != null) {
                    mDrag2LoadAction.load(Pull2RefreshListView.this);
                } else {
                    Pull2RefreshListView.this.finishLoading();
                }
            }
        }
    ...
    public interface Pull2RefreshListener {
        void onRefresh(Pull2RefreshListView refreshableListView);
    }

    public interface Drag2LoadListener {
        void load(Pull2RefreshListView refreshableListView);
    }
