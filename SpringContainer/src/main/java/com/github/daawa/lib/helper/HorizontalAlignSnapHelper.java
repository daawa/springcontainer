package com.github.daawa.lib.helper;
...
/**
 * Created by hzzhangzhenwei on 2018/1/9.
 *
 * Align the first visible item  to the left of recycler view.
 */

public class HorizontalAlignSnapHelper extends SnapHelper {
    ...
    private View findStartView(RecyclerView.LayoutManager layoutManager, OrientationHelper helper) {
        if (layoutManager instanceof LinearLayoutManager) {
            ...
            // the last view is exposed. no aligning for the first
            ...
            } else if(firstChildPosition + 1 < layoutManager.getItemCount()){
    ...
    public int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX, int velocityY) {
        ...
        int itemCountPerScreen = layoutManager.getWidth() / getHorizontalHelper(layoutManager).getDecoratedMeasurement(currentView);

        int hDeltaJump;
        if (layoutManager.canScrollHorizontally()) {
            hDeltaJump = estimateNextPositionDiffForFling(layoutManager, getHorizontalHelper(layoutManager), velocityX, 0);

            if (hDeltaJump > itemCountPerScreen) {
                hDeltaJump = itemCountPerScreen;
            }
            if (hDeltaJump < -itemCountPerScreen) {
                hDeltaJump = -itemCountPerScreen;
            }
